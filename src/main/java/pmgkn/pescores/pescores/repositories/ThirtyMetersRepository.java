...
public interface ThirtyMetersRepository extends JpaRepository<ThirtyMetersEntity, UUID> {
    ...
    @Query("SELECT tm FROM DenseBallEntity as tm WHERE tm.classNum = :n AND tm.gender = :g AND :r >= tm.min AND :r <= tm.max")
    ThirtyMetersEntity getDenseBallEntityByResult(@Param("n") Integer number,
