...
public interface TwoHundredMetersRepository extends JpaRepository<TwoHundredMetersEntity, UUID> {
    ...
    @Query("SELECT th FROM DenseBallEntity as th WHERE th.classNum = :n AND th.gender = :g AND :r >= th.min AND :r <= th.max")
    TwoHundredMetersEntity getDenseBallEntityByResult(@Param("n") Integer number,
